...
def better_normalize_label(label):
    if not label:
        return 'unknown'
    label = label.lower().replace('cells', '').replace('cell', '').replace(' ', '').replace('+', '')
...
for (model_name, filepath) in uploaded_files.items():
    ...
    for cluster in cluster_data:
        method = cluster['method']
        if method not in methods_dict:
            methods_dict[method] = {'ground_truth': [], 'predicted': []}

        top_genes = cluster.get('top_genes', [])
        predicted_label = cluster.get('cell_type_prediction', 'unknown')
        _, _, broad_gt = assign_ground_truth_fuzzy_pure(top_genes)
    ...
    for method, data in methods_dict.items():
        conf_matrix = pd.crosstab(pd.Series(data['ground_truth'], name='Ground Truth'),
        ...
        plt.tight_layout()
        plt.savefig(f"outputs/confusion_matrix_{model_name}_{method}.png")
        plt.close()

# --- Plot polished and sorted model accuracy comparison ---
...
plt.ylim(0, 100)
plt.title("Model and Method Accuracy Comparison (Sorted)", fontsize=16)
...
plt.tight_layout()
plt.savefig("outputs/model_accuracy_comparison_polished_sorted.png", dpi=300)
plt.show()

# --- Plot polished and sorted model accuracy comparison (WITHOUT Inflection) ---

# Filter out inflection
...
plt.ylim(0, 100)
plt.title("Model and Method Accuracy Comparison (Sorted, no Inflection)", fontsize=16)
plt.ylabel("Accuracy (%)", fontsize=14)
plt.xlabel("")
plt.xticks(rotation=45, ha='right', fontsize=12)
plt.yticks(fontsize=12)
plt.legend(title="Method", title_fontsize=12, fontsize=10)
plt.tight_layout()
plt.savefig("outputs/model_accuracy_comparison_polished_sorted_no_inflection.png", dpi=300)
plt.show()
