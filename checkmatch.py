            ...
            for j, model2 in enumerate(model_names):
                if i <= j:  # only upper triangle + diagonal
                    ...
                    if match_type == "hard":
                        ...
                        matches = sum(
                            compute_similarity(models[model1].get(cluster, ""), models[model2].get(cluster, ""))
                            for cluster in set(models[model1].keys()).union(models[model2].keys())
                        )
                    else:
...
def plot_heatmaps(supertable, title_suffix):
    ...
    for ax, method in zip(axes, ["standard", "kneed", "inflection"]):
        ...
        ax.set_xlabel("Model")
        ax.set_ylabel("Model")
        ax.set_yticklabels(ax.get_yticklabels(), rotation=0)  # <- Make y-axis labels horizontal
...
hard_supertable.to_csv("plots/hard_supertable.csv", index=False)
plot_heatmaps(hard_supertable, "(Hard Match)")

# Soft matching (fuzzy token matching)
soft_supertable = build_supertable(method_model_predictions, match_type="soft")
soft_supertable.to_csv("plots/soft_supertable.csv", index=False)
plot_heatmaps(soft_supertable, "(Soft Fuzzy Match)")
